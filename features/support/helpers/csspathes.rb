class Csspathes
  ...
  def initialize
    ...
    @navigation_menu_title_path = case
      when ENV['SHOP'].match(/chefworks/) && ENV['COUNTRY'].match(/de/) then      '.page-wrap > nav'
      when ENV['SHOP'].match(/export/) && ENV['COUNTRY'].match(/[a-z]{2}/) then   '.page-wrap > nav'
      when ENV['SHOP'].match(/pulsiva/) && ENV['COUNTRY'].match(/it/) then  '.navigation--list-wrapper > ul > li:first-child > a'
      when ENV['SHOP'].match(/pulsiva/) && ENV['COUNTRY'].match(/[a-z]{2}/) then  '.navigation--list-wrapper > ul > li:first-child > a'
    ...
    @navigation_sidebar_sub_path = case 
      when ENV['SHOP'].match(/pulsiva/) && ENV['COUNTRY'].match(/it/) then      '.has--sub-categories > ul > li:nth-child(1)'
      when ENV['SHOP'].match(/pulsiva/) && ENV['COUNTRY'].match(/[a-z]{2}/) then      '.sidebar--categories-wrapper > div.sidebar--categories-navigation > ul > li.navigation--entry.is--active.has--sub-categories'
    ...
    @account_payment_cashOnDelivery_path = case 
      ...
      when ENV['SHOP'].match(/vega/) && ENV['COUNTRY'].match(/de/) then            '#payment_mean5'
      ...
      when ENV['SHOP'].match(/v_jobeline/) && ENV['COUNTRY'].match(/de/) then      '#payment_mean5'
    ...
    @checkout_panel_path = case 
      ...
      when ENV['SHOP'].match(/pulsiva/) && ENV['COUNTRY'].match(/[a-z]{2}/) then   'section > div > div > div > div > div.panel.has--border'
